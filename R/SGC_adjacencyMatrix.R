...
checkNumeric = function(x, stp){
  ...
  res = which(sapply(x, class) != "numeric")
...
################################################################################# adjacencyMatrix
adjacencyMatrix = function(expData, calibration = FALSE, normalization = TRUE,
                           tom = TRUE, saveAdja = FALSE,
                           adjaNameFile = "adjacency.RData",
                           hm = "adjaHeatMap.png", seed = NULL){

  # calculate the adjacency matrix of the network
  #
  # Arguments:
  #       expData: a dataframe or matrix containing the expression data.
  ...
  #       saveAdja : TRUE/FALSE
  #                   if TRUE, the adjancency matrix will be saved as *.RData
  #       adjaNameFile: string indicates the name of file for saving adjancency matrix
  #
  #       hm: string indicates the name of file for saving adjancency matrix heat map
  #
  # Value:
  #       adja: matrix of dimension n*n of the network
  ...
  if(saveAdja){
    if(!is.character(adjaNameFile)){
      warning("adjaNamFile is not string", call. = FALSE)
        message(paste0("using following name \n ",  " adjacency.RData"))
        adjaNameFile = "adjacency.RData"}
  ...
  if(!is.null(hm)){
    hm_plt = SGC_plot_heatMap(adja, tit = "Adjacency Heatmap",
