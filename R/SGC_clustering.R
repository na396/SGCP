...
findNoise = function(evec, thresh = 20){
  ...
  if(length(v2) < 2*thresh){
    stop(paste0("not enough genes, total number of genes is ", length(v2)))
  }
  ...
  if(p > thresh && n > thresh){
    ...
    message("you need to run SGC on each components by yourself")
...
clusterNumber = function(egvals, maxNum = 102){
  ...
  # method: eigen gap
  k = seq(2, maxNum)
  pairDiff = abs(diff(egvals[1:maxNum]))
  ...
  egvals = 1 - egvals
  egvals = egvals[1:min(maxNum, length(egvals))]
  ...
  message(paste0("number of clusters for relativeGap method is "), optf)
  message(paste0("number of clusters for secondOrderGap method is "), opts)
  message(paste0("number of clusters for additiveGap method is "), optg)
    ...
    pltgap = ggplot(data = dfgap, aes(x = k, y = pairDiff)) +
      ...
      labs(x = "cluster number", y = "Additive Gap",
           title = "Additive Gap Method")
    ...
    pltfirst = ggplot(data = df, aes(x = kfirst, y = firstOrder)) +
      ...
      theme(legend.text = element_text(size = 7))+
      theme(legend.position = c(.9, .9)) +
      labs(x = "cluster number", y = "Relative Gap Method",
           title = "Relative Gap Method ")
    ...
    pltsecond = ggplot(data = df, aes(x = ksecond, y = secondOrder)) +
      ...
      theme(legend.text = element_text(size = 7))+
      theme(legend.position = c(.9, .9)) +
      labs(x = "cluster number", y = "Second-Order Gap",
           title = "Second-Order Gap Method")
...
cvConductance = function(adja, k, Y, X, func = min,
  ...
  Yf = Y[, 1: min(2*krelativeGap, ncol(Y))]
  Xf = X[1:min(2*krelativeGap, length(X))]
  ...
  Ys = Y[, 1: min(2*ksecondOrderGap, ncol(Y))]
  Xs = X[1:min(2*ksecondOrderGap, length(X))]
  ...
  Yg = Y[, 1: min(2*kadditiveGap, ncol(Y))]
  Xg = X[1:min(2*kadditiveGap, length(X))]
  ...
  message(paste0("selected method in cvConductance is ", resmethod ))
...
sigClusGO = function(adja, Y, X, k, annotation, geneID,
  ...
  Yt = Y[, 1: min(2*k, ncol(Y))]
  Xt = X[1:min(2*k, length(X))]
...
clustering = function(adjaMat, geneID , annotation_db ,
  ...
  # Calculates the spectral transformation on adjacency symmetric squared matrix
  #
  # Arguments:
  #           adjaMat: adjacency matrix of n*n where n is the number of the genes,
  #                  this matrix is squared symmetric with values in (0, 1)
  #                  and 0 diagonal
  #           geneID:a vector of size n where n is the number of genes
  #               		containing the genes IDs
  #           annotation_db: a string indicating the genomic wide annotation database
  #           kopt: an integer denotes the optimal number of clusters k by the user,
  # 	             default is NULL
  #           method: method for identifying the number of clusters k, default NULL,
  #               	either "relativeGap"", "secondOrderGap", "additiveGap", or NULL
  #			  func.GO: a function for gene ontology validation,
  #                  default is sum
  #			  func.conduct: a function for conductance validation,
  #                      default is min
  #           maxIter: an integer,
  #                   identifies the maximum number of iteration for kmeans
  #           numStart: a number
  #                     identifies the number of start for kmeans
  #           saveOrig: boolean, default FALSE,
  #                    if TRUE, keeps the transformation matrix
  #           n_egvec: either "all" or an integer,
  #                 indicates the number of columns of transformation matrix to be kept
  #           sil: boolean, default TRUE,
  ...
  if(!is.null(seed) & !is.numeric(seed)){
    warning("seed must be numeric", call. = FALSE)
    message("making seed to NULL")
    seed = NULL}
  ...
  if(is.null(geneID)){
    geneID = paste0(rep("gene", ng), seq(1:ng))}
  ...
  if(length(setdiff(method, c("relativeGap",
    ...
    warning("method can be either relativeGap, secondOrderGag, or additiveGap", call. = FALSE)
  ...
  if(sum(round(X, 7) == 1) > 1 ){
    ...
    message(paste0("number of noisy genes are... ", length(nois_ind)))
  ...
  if(round(constant, 7) != 1 ){
    ...
    print(paste0("the condition value is ",
  ...
  if(saveOrig == TRUE){
    ...
    message(paste0("n_egvec is ", n_egvec))
    Yorig = Y[, 1:min(n_egvec, ncol(Y))]
    Xorig = X[1:min(n_egvec, length(X))]
 ...
 #################################################################################
  ...
  if(is.null(kopt) && is.null(method) && !is.null(annotation_db)){
    ...
    # optimal by user
    Yopt = Y[, 1: min(2*kopt, ncol(Y))]
    Xopt = X[1:min(2*kopt, length(X))]
