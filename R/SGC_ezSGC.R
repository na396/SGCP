...
################################################################################# ezSGC
ezSGC = function(expData, geneID, annotation_db, semilabel = TRUE,
  ...
  if(dir %!in% c("under", "over") || length(dir) > 2 ){
    warning("dir must be in c(under or over) \n making to default",
            call. = FALSE)
    dir = c("over", "under")}

  if(onto %!in% c("BP", "CC", "MF") || length(onto) > 3){
  ...
  resAdja = adjacencyMatrix(expData = expData,
                            calibration = calib, normalization = norm, tom = tom,
  ...
  ################################################################################ geneOntology
    ...
    initial = data.frame(geneID = resClus$geneID, initialClusters = resClus$clusterLabels)
    final = data.frame(geneID = geneID, finalClusters = resSup$FinalLabeling$FinalLabel)
    ...
    if(length(remain)== 1){
      message(paste0("cluster ", remain, " is wiped out"))
    }else if(length(remain)){
      message(paste0("clusters", remain, " are wiped out"))}
