...
################################################################################# semiLabeling
semiLabeling = function(geneID, df_GO, GOgenes, cutoff = NULL, 
                        percent = 0.10, stp = 0.01){
  
  # performs semiLabeling, identifies significant genes and labels them 
  #                         correspond to their cluster label
  #
  # Arguments:
  #           geneID: a vector containing the genes IDs of size n
  #                where n is the number of genes 
  #           df_GO: GOresults dataframe returned by geneOntology function,
  #               	consists the information of GO terms the clusters
  #           GOgenes: FinalGOTermGenes list returned by geneOntology function,
  #                 is a list of genes associated to the GO Terms per each cluster
  #           cutoff: a numeric in (0, 1) default NULL,
  #                is a base line for GO term significancy
  #           percent: a number in (0,1) default 0.1,
  #                    indicate the percentile for finding top GO terms
  #           stp: a number in (0,1) default 0.01,
