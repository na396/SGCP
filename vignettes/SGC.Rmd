...
## `SGC` installation
To install `SGC` please visit [here](https://github.com/na396/SGC). Load `SGC` after installation.
...
For illustrative purposes we will use an example of a gene expression provided with `SGC`.
...
[link](https://bioconductor.org/packages/release/data/annotation/html/org.Hs.eg.db.html). Let look at the input example.
...
As it is seen, expression data have 1000 genes and 5 samples. Let's start.
...
# geneID must be a vector, so convert the dataframe to a vector
geneID <- geneID$geneEntrezID
head(geneID)
length(geneID)
...
```{r, message=TRUE, warning=FALSE}
sgc <- ezSGC(expData = expData, geneID = geneID, annotation_db = annotation_db, sil = TRUE, hm = NULL)
```
