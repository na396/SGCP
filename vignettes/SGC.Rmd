...
1. __Network Construction__ step (`adjacencyMatrix` function)
    ...
    * `norm`: boolean, default TRUE, if TRUE `SGC` divides each genes (rows) by its norm2.
...
At the end, `SGC` performs on more Gene Ontology Enrichment on the final modules.
...
* __expData__ is a matrix or a dataframe of size `m*n` where `m` and `n` are the number of genes and samples respectively and it can be either DNA-microarray or RNA-seq . In another words, in `expData`, rows and columns correspond to genes and samples respectively and the entry `i,j` is an expression value for gene `i` in sample `j`. `SGC` does not perform any normalization or correction for batch effects and it is assumed that these 
...
For illustrative purposes we will use an example of a expression matrix provided with `SGC`. 
...
Here SGC to ease the computation, 1500 genes that have the most variance selected with 5 samples have been selected . We call this matrix cheng, and rownames in this matrix shows the gene Entrez ids of its corresponding row. 
...
Note that you need to the annotation_db installed on your system. In this exampel, you need to `org.Hs.eg.db` installed.
...
[here](## `SGC` Pipeline Parameters and Workflow) except `calib`
...
sgc = ezSGC(expData = expData, geneID, annotation_db = annotation_db, sil = TRUE, seed = 123)
```
As it is seen, `SGC` print out information of each step. 
...
sgc_plts = SGC_ezPLOT(sgc = sgc, expreData = expData, silhouette_index = TRUE, 
                      keep = TRUE)
```
...
Here, we will skip the detail of function's input and output as it is described in the help document. `SGC` allows user to visualize the PCA of the input gene expression data using `SGC_plot_pca` function.

```{r, message=TRUE, warning=FALSE}
message("Plotting PCA of expression data")
pl = SGC_plot_pca(m = expData, clusLabs = NULL, tit = "PCA plot", ps = .5)
print(pl)
```
...
resAdja = adjacencyMatrix(expData = expData, seed = 123)
```

`resAdja` is the adjacency matrix of the gene co-expression network. We can visualize the heatmap of the adjacency matrix using$~$` SGC_plot_heatMap` function as follow.
...
Code below shows how to use this function.
...
pl = SGC_plot_pca(m = expData, clusLabs = NULL, tit = "PCA plot without label", ps = .5)
print(pl)
...
pl = SGC_plot_pca(m = resClus$Y, clusLabs = NULL, tit = "PCA plot without label", ps = .5)
print(pl)
...
pl = SGC_plot_pca(m = resClus$Y, clusLabs = resClus$clusterLabels, tit = "PCA plot with label", ps = .5)
print(pl)
...
Previously, we saw that the cluster with minimum conductance index in method "relativeGap" has higher enrichment of corresponding clusters in other method. Interestingly, both clusters from "relativeGap" also have lower conductance index in compare with other method. This shows that gene ontology validation agrees with conductance validation, and clusters with lower conductance index have higher enrichment. We discussed bout this observation in detail [here](https://arxiv.org/abs/2209.10545)
...
Silhouette index ranges between -1 and 1, and the closer to 1, the better the gene is explained by the cluster. As it is seen, majority of genes have silhuette index very close to 1 which means that genes are well-described by the clusters. Interestingly, in worse case scenario, genes have zero silhouette index (the gap in the plot), and there is no negative index for any gene.


### `geneOntology` function
__Gene Ontology Enrichment__ step is implemented using `geneOntology` function. This function returns the gene ontology enrichment on the clusters as `GOresults`. It also returns the `FinalGOTermGenes` list which identifies the gene ID correspond to the GO terms found `GOresults`. Code below shows how to use this function.
...
`SGC` has four functions that illustrate the gene ontology enrichment result; `SGC_plot_jitter`, `SGC_plot_density`, `SGC_plot_bar`, and `SGC_plot_pie`. Note that p-values are log-transformed, and therefore, The higher the point, the more signifcant the GO term is. 
...
pl = SGC_plot_pie(df = resInitialGO$GOresults, tit = "Initial GO Analysis",
...
```
Above table shows the number of remarkable genes per cluster, and we say a cluster is remarkable if it has at least one remarkable gene. Therefore, both clusters are remarkable here. However, we found out if a cluster is not remarkable, it will be wiped out through the __Semi-labeling__ and __Semi-supervised__ steps.
...
```
Table above shows the unremarkable gene assignment to each cluster.
...
```{r, message=TRUE, warning=FALSE}
message("Plotting PCA of transformed data with label")
pl = SGC_plot_pca(m = resClus$Y, clusLabs = resSemiSupervised$FinalLabeling$FinalLabel, 
             tit = "PCA plot with label", ps = .5)
print(pl)
```
...
Comparing this result with initial clusters, it is observed that gene ontology enrichment has been increased slightly. We found out, if all initial clusters are remarkable, like here, __Semi-labeling__ and __Semi-supervised__ steps does not change the genes cluster labels fundamentally, and initial clusters converges to final clusters. Otherwise, `SGC` will wiped out the non-significant clusters and increases the enrichment of remarkable clusters. And, final modules are different from initial clusters. You can see the detail in manuscript [SGC manuscript](https://arxiv.org/pdf/2209.10545.pdf).
