...
For a comprehensive understanding of `SGCP`, including detailed methodology and results, refer to the manuscript available [here](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-024-05848-w).

## `SGCP` installation
To install the package `SGCP` package and its dependencies, follow these steps. For more information please visit [here](https://github.com/na396/SGCP)). In this guide, we demonstrate the application of SGCP using two key packages: `SummarizedExperiment` and `org.Hs.eg.db`.
...
`SGCP` requires three main inputs: gene expression data (`expData`), gene identifiers (`geneID`), and a genome-wide annotation database (`annotation_db`).


* __`expData`__:  A matrix or dataframe of size `m*n`, where `m` is the number of genes and `n` is the number of samples. It can be either DNA-microarray or RNA-seq data. In `expData`, rows represent genes, columns represent samples, and the entry at position `(i, j)` is the expression value for gene `i` in sample `j.`  Note that `SGCP` assumes that pre-processing steps, such as normalization and batch effect correction, have already been performed, as these are not handled by the pipeline.

* __`geneID`__: A vector of size m, where each entry at index `i` denotes the gene identifier for gene `i`. There is a one-to-one correspondence between the rows in `expData` and the entries in the `geneID` vector, meaning index `i` in `geneID` corresponds to row `i` in `expData.`

* __`annotation_db`__: The name of a genome-wide annotation package for the organism of interest, used for the gene ontology (GO) enrichment step. The `annotation_db` must be installed by the user before using `SGCP.` 
...
* each gene must have exactly one unique identifier, specified by `geneID`.
* Genes must be annotated with Gene Ontology (GO) terms.


Note that `SGCP` relies on the on [GOstats](https://bioconductor.org/packages/release/bioc/html/GOstats.html) package
for GO enrichment analysis. Therefore, both `geneID` and `annotation_db` must  adhere to the standards required by `GOstats`.
...
for more information see ([Cheng et al.](https://www.sciencedirect.com/science/article/abs/pii/S0010482520303061?via%3Dihub)). For computational efficiency, we have selected 1000 genes with the highest variance and 5 samples from the original dataset.
...
Now, let's examine the input example.
...
2. __Network Clustering__ step (`clustering` function) 
    ...
    * `method`:  Either "relativeGap", "secondOrderGap", "additiveGap", or `NULL` (default), 
    Defines the method for finding the number of cluster.
...
3. __Gene Ontology Enrichment__ step (`geneOntology` function)
    ...
    * `hgCutoff`: A numeric value between `0` and `1`, default is `0.05`. GO terms with p-values smaller than                `hgCutoff` are retained.
...
4. __Gene Semi-labeling__ step (`semiLabeling` function)
    * `cutoff`: a numeric between 0 and 1, default is  `NULL`. This baseline is used to determined the significance of GO terms, distinguishing between remarkable and unremarkable genes.
    * `percent`: a number between 0 and 1, default is `0.1`. Indicates the percentile used to identify top GO terms.
    * `stp`: a number between 0 and ,1 default is `0.01`. Specifies the increment added to the `percent` parameter.
...
Detailed of the steps are available in the manuscript in [SGCP](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-024-05848-w). 
...
method that `func.conduct` (default `min`) on its cluster is minimum. Otherwise (`kopt`, `method` are `NULL`, but `annotation_db` and `geneID` are not `NULL`), 
...
For a comprehensive description of each parameter, refer to the [manuscript](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-024-05848-w).
...
`SGCP` offers visualization capabilities to users. The `SGCP_ezPLOT` function accepts the `sgcp` result from `ezSGCP`, along with `expData` and `geneID` inputs. It generates two files, an Excel spreadsheet and a PDF, depending on the initial call. Users can also retain the plotting object by setting `keep = TRUE`. In this example, we enable `silhouette_index` to visualize the silhouette index plot.
...
```
In [here](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-024-05848-w) We observed that clusters with lower conductance indices tend to exhibit higher enrichment. This analysis confirms our observation. In the clustering function, gene ontology enrichment is compared among clusters with the minimum conductance for the methods "relativeGap" (`cluster rg1`), "secondOrderGap" (cluster `sg4`), and "additiveGap" (cluster `ag1`). Among them, cluster `rg1` shows higher enrichment. Interestingly, this cluster also demonstrates a lower conductance index compared to `ag1` and `sg4.`
...
Uncomment and run the following code to perform gene **Semi-supervised** step.
