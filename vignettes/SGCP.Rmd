...
message("rownames and colnames correspond to gene Entrez ids and sample names")
head(assay(cheng))

message(" \n gene ids...")
message("rownames are the gene Entrez ids")
head(rowData(cheng))
```
...
# Run `SGCP`
There are two options to run `SGCP`. 
1 `ezSGCP` function: which implements all steps in a single function.
...
```{r, message=TRUE, warning=FALSE}
sgcp <- ezSGCP(expData = expData, geneID = geneID, annotation_db = annotation_db, sil = TRUE, hm = NULL)
```
...
sgcp_plts <- SGCP_ezPLOT(sgcp = sgcp, expreData = expData, silhouette_index = TRUE, 
                      keep = TRUE)
```
...
message("plotting initial GO p-values jitters...")
      pl <- SGCP_plot_jitter(df = resInitialGO$GOresults, tit = "Initial GO p-values",
...
```{r, message=TRUE, warning=FALSE}
message("Plotting PCA of transformed data with label")
pl <- SGCP_plot_pca(m = expData, clusLabs = resSemiSupervised$FinalLabeling$FinalLabel, 
...
```{r, message=TRUE, warning=FALSE}
message("Plotting PCA of transformed data with label")
pl <- SGCP_plot_pca(m = resClus$Y, clusLabs = resSemiSupervised$FinalLabeling$FinalLabel, 
             tit = "PCA plot with label", ps = .5)
print(pl)
```
